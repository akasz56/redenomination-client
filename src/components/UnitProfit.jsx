...
export default function UnitProfit({ profits, budget, isSession = false }) {
  ...
  function SessionElement() {
    ...
    const totalProfit = sumByUsername(profits).reduce(
      ...
      <>
        <CSVLink
          ...
          data={[
            ...
            ...sellers.map((item) => [
              item.username,
              displayPrice(parseInt((item.profit / totalProfit) * budget)),
            ]),
            ...buyers.map((item) => [
              item.username,
              displayPrice(parseInt((item.profit / totalProfit) * budget)),
            ...
            <div key={i} className="d-flex justify-content-center mb-3">
              ...
              <Form.Control
                ...
                defaultValue={new Intl.NumberFormat("id-ID", {
                  style: "currency",
                  currency: "IDR",
                }).format((item.profit / totalProfit) * budget)}
              />
            </div>
          ))}
        </div>
  ...
  function SimulationElement() {
    ...
    const totalProfit = useMemo(
      () => usernames.reduce((sum, item) => sum + item.profit, 0),
      [usernames]
    );
    const usernames2 = useMemo(() => usernames, [usernames]);
    const usernames1 = useMemo(
      () => usernames2.splice(0, parseInt(usernames.length / 2)),
  ...
  return (
    <section className="row">
      <h1 className="text-center" style={{ marginTop: "7rem" }}>
        Reward peserta Simulasi
      </h1>
      <hr />

      {isSession ? <SessionElement /> : <SimulationElement />}
    </section>
  );
}
