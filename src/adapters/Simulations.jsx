...
export async function createSimulation(body, additionalPlayer) {
  ...
  body.buyer = Object.keys(body.unitValue).map((key) => {
    return { unitValue: parseInt(body.unitValue[key]) };
  });
  body.seller = Object.keys(body.unitCost).map((key) => {
    return { unitCost: parseInt(body.unitCost[key]) };
  });
