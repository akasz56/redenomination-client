...
export default function Participants() {
  ...
  if (state === null) {
    alertUserSocket({
      status: "000",
      message: "Anda belum terdaftar, silahkan login dahulu",
    });
    window.location.href = "/";
  }
  const [stateData, setStateData] = useState(state);
  const [stateStage, setStateStage] = useState(
    stateData.sessionData.phaseId === participantStage.READY
      ? participantStage.READY
      : participantStage.SIMULATION
  );

  useEffect(() => {
    function retryLogin() {
      socket.emit("loginToken", {
        token: state.detail.loginToken.toUpperCase(),
      ...
      function retryLoginHandler(res) {
        if (res.status === 200 && res.data.isSessionRunning) {
          ...
          setStateStage(
            res.data.sessionData.phaseId === participantStage.READY
              ? participantStage.READY
              : participantStage.SIMULATION
          );
  ...
  switch (stateStage) {
    case participantStage.READY:
      return (
        <ReadyScreenHandler data={stateData} setStateStage={setStateStage} />
      );
