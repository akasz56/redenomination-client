        ...
        if (stage === 'ready') {
            ...
            socket.on("collectedProfit", res => {
                ...
                if (res.profitCollection.length === data.participantNumber) {
                    ...
                    const myReward = Math.round((myTotalProfit / allProfit) * res.simulationBudget)
...
function PostedOfferHandler(stage, timer, socket, data, phaseContinue) {
    switch (stage) {
        ...
        case "complete":
        case "otwComplete":
            if (data.role === "seller") { return <CompleteScreen data={data} /> }
            else if (data.role === "buyer") { return <CompleteScreen data={data} /> }
            else { return <BlankScreen lineNumber="193" /> }
...
function DoubleAuctionHandler(stage, setStage, socket, data, phaseContinue) {
    switch (stage) {
        case "auctionScreen":
            if (data.role === "seller") { return <SellerAuctionScreen socket={socket} data={data} setStage={setStage} phaseContinue={phaseContinue} /> }
            else if (data.role === "buyer") { return <BuyerAuctionScreen socket={socket} data={data} setStage={setStage} phaseContinue={phaseContinue} /> }
        ...
        case "otwComplete":
            ...
            else { return <BlankScreen lineNumber="210" /> }
...
function DecentralizedHandler(stage, setStage, socket, data, phaseContinue) {
    switch (stage) {
        case "postPriceDS":
        ...
        case "listShops":
            if (data.role === "seller") { return <SellerIdleDS socket={socket} data={data} setStage={setStage} phaseContinue={phaseContinue} /> }
            else if (data.role === "buyer") { return <Lobby socket={socket} data={data} setStage={setStage} phaseContinue={phaseContinue} /> }
        ...
        case "otwComplete":
            ...
            else { return <BlankScreen lineNumber="232" /> }
...
function CompleteScreen({ data }) {
    const kontak = {
        nomorwa: "6289608703393"
    }

    function generateLink() {
        return "https://wa.me/" + kontak.nomorwa + "?text=Saya menerima reward sebesar " + data.rewards + " dalam simulasi " + data.simulationType + " (" + data.loginToken + ")"
    }
    ...
    return (
        <Container className='text-center d-flex flex-column'>
            ...
            <p className='mt-5'>
                ...
                <span className='text-primary fw-bolder fs-1 d-block'>Rp. {data.rewards}</span>
...
function BlankScreen({ lineNumber }) {
    return (
        <div className="child">
