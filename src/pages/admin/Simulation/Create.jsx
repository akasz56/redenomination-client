...
export default function Create() {
  ...
  function additionalPlayerHandler(e, isSeller) {
    ...
    if (isSeller) {
      setUnitValues({
        ...unitValues,
        ["pembeli" + (formData.participantNumber / 2 + 0.5)]: undefined,
      });
      setUnitCosts({
        ...unitCosts,
        ["penjual" + (formData.participantNumber / 2 + 0.5)]: value,
      });
    } else {
      setUnitCosts({
        ...unitCosts,
        ["penjual" + (formData.participantNumber / 2 + 0.5)]: undefined,
      });
      setUnitValues({
        ...unitValues,
        ["pembeli" + (formData.participantNumber / 2 + 0.5)]: value,
      });
    }
  }
        ...
        <form onSubmit={submitForm}>
              ...
              <Form.Group controlId="participantNumber">
                ...
                <Form.Control
                  ...
                  onChange={(e) => {
                    setFormData({
                      ...formData,
                      participantNumber: e.target.value,
                    });
                  }}
                />
                &nbsp;Maka,
                <span className="fw-bold">
                  {" "}
                  {togglePlayer
                    ? Math.floor(formData.participantNumber / 2) +
                      (formData.participantNumber % 2)
                    : Math.floor(formData.participantNumber / 2)}{" "}
                  Penjual{" "}
                </span>
          ...
          <section className="row mt-5">
            <div className="col-md-6">
              ...
              {Array.from({ length: formData.participantNumber / 2 }).map(
                (_, i) => (
                  <UnitInput
                    key={i + 1}
                    id={i + 1}
              ...
              {formData.participantNumber % 2 ? (
                togglePlayer ? (
                  <UnitInput
                    ...
                    role="penjual"
                    onChange={(e) => {
                      additionalPlayerHandler(e, true);
                    }}
                  />
                ) : (
              ...
              {formData.participantNumber % 2 ? (
                !togglePlayer ? (
                  <UnitInput
                    ...
                    role="pembeli"
                    onChange={(e) => {
                      additionalPlayerHandler(e, false);
                    }}
                  />
                ) : (
                  <div className="d-flex">
